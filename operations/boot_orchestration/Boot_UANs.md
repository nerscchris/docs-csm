...
1. (`ncn-mw#`) Create a BOS session to boot the UAN nodes.
    ...
    ```bash
    cray bos v2 session create --template-name uan-sessiontemplate-PRODUCT_VERSION \
...
1. (`ncn-mw#`) List the CFS sessions.
    ...
    If CFS was enabled in the boot session template, the BOS will set configuration in CFS that will trigger a CFS session.
